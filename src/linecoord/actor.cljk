(ns linecoord.actor
  "LineCoordActor — the ISCO-08 7413 electrical line installers and
  repairers crew dispatch scheduling/logistics coordination actor as
  ...
  resume after human sign-off. This actor coordinates CREW/DISPATCH
  SCHEDULING/LOGISTICS ONLY — it never performs power-line work
  itself and never commits a proposal the LineCoordGovernor refuses.
  Modeled on cloud-itonami-isco-7214's steelcoord.actor.
  ...
  The unconditional invariant: the Electrical Line Crew Coordination
  Advisor can never directly commit a coordination record the
  LineCoordGovernor refuses — every commit-record! call is gated
  behind `:decide`, and the governor's scope-exclusion rule makes any
  proposal to finalize power-line-work execution, authorize a
  de-energization/re-energization/lockout clearance, or override a
  utility-safety officer's judgment an unconditional, permanent
  :hold."
...
(defn build-graph
  "Build a compiled LineCoordActor graph. `store` implements
  `linecoord.store/Store`. `advisor` implements
  `linecoord.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:circuit-id (:circuit-id request)
                                    :op (:op proposal)
...
(defn approve!
  ...
  can never be used to approve a power-line-work-execution
  finalization, a de-energization/re-energization/lockout-clearance
  authorization, or a utility-safety-officer-judgment override."
  [graph thread-id]
